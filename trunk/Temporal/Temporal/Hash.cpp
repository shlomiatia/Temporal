...
namespace Temporal
{
	const Hash Hash::INVALID = Hash(0u);
	...
	unsigned int crc32(const char* buffer)
	{
		unsigned int result = ~0;
		const char* bufferIterator = buffer;
		int remaining = strlen(bufferIterator);
		for (; remaining--; ++bufferIterator)
			result = ( result >> 8 ) ^ kCrc32Table[(result ^ *bufferIterator) & 0xff];
		return ~result;
	}
	...
	void HashToString::save(const Hash& key, const char* string)
	{
		...
		{
			const char* previousString = i->second;
