...
namespace Temporal
{
	...
	{
	public:
		SlopedArea(const Point& center, const Vector& slopedRadius, float yRadius) : _center(center), _slopedRadius(slopedRadius), _yRadius(yRadius) {}
		...
		SlopedArea resize(const Vector& delta) const { return SlopedArea(getCenter(), getSlopedRadius().normalize() * (getSlopedRadius().getLength() + delta.getVx() / 2.0f), getYRadius() + delta.getVy() / 2.0f); }
	private:
		Point _center;
